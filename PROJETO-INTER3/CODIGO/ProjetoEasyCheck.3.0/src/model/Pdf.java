	...
	public void gerarDocumento(String nomeEscola, String nomeFornecedor, int idNotaFiscal, List<Produto> produtosNF, int itensNF) throws SQLException {
		...
		try{			
			...
			for(int j = 0; j < produtosNF.size(); j++) {
				...
				documento.add(new Paragraph("\t                                                                       Qtd :"+produtosNF.get(j).getQtd()+"\t   Preço unit.:  "+produtosNF.get(j).getPreco_uni() +"\t	Preço Total "+precoTotal));   
				documento.add(new Paragraph("----------------------------------------------------------------------------------------------------------------------------------------"));
				
			}
			documento.add(new Paragraph("----------------------------------------------------------------------------------------------------------------------------------------"));
			documento.add(new Paragraph("\t										Número de itens da NF: "+itensNF+ "                                                      Preço Total: "+ precoFinal));
			documento.add(new Paragraph("----------------------------------------------------------------------------------------------------------------------------------------"));
